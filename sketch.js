...
async function initMediaPipe() {
  try {
    ...
    isModelReady = true;
    if (statusBadge) statusBadge.html("✅ 준비 완료! 마퀸 조작 제스처를 학습시키세요.");
...
function classifyKNN(features) {
  ...
  const displayLabel = commandEmoji[label] || label;
  resultLabel.html(displayLabel);
  resultLabel.style("color", conf >= 0.85 ? "#00E676" : "#FFEB3B");
  resultConf.html(`정확도: ${(conf * 100).toFixed(0)}%`);

  let displayMsg = `📡 전송: ${displayLabel}`;
